# CW1b: LLVMlite backend

Quick Start:

1. open the folder in VSCode (re-open in the dev container)
...
files, it will compile the .ll files to .s files (using the AICE2007
...
using clang instead of the AICE2007 backend, which can be useful for
testing purposes.
...
* To compile ll files using the AICE2007 backend:
...
* Useful flags:
  ...
  | --execute-x86     | runs the resulting a.out file natively (for both 4521/5521 backend and clang-compiled code) |
...
* Example uses:
  ...
  Run the test case llprograms/factrect.ll using the 4521/5521 backend:
