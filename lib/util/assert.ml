(* CIS 4521 Assertion Testing and Grading Infrastructure -------------------- *)
...
let assert_eqfs f v2 : assertion =
 fun () ->
  ...
  then failwith @@ Printf.sprintf "not equal\n\texpected:%s\n\tgot     :%s\n"
                     (String.escaped v2) (String.escaped s1)
  else ()
...
(* Number of digits of precision for a float x. Argument p is the number of
   decimal places desired (must be at least 1) *)
let prec_digits p x = (int_of_float @@ floor @@ log10 x) + (1 + p)

(* returns
   (name_pts, passed, failed, total, points_earned, max_given, max_hidden) *)
let get_results (t : result test) =
  ...
  | GradedTest (name, pts, cases) ->
     ...
     if total > 0 
     then
       let points_earned = ((float_of_int passed) /. (float_of_int total))
                           *. (float_of_int pts) in
       let name_pts = 
         Printf.sprintf "%s (%1.*g/%d points = %d/%d tests)"
           name
           (prec_digits 1 points_earned)
           points_earned
           pts
           passed
           total 
