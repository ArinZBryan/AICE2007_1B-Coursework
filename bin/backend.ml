...
let compile_terminator (fn:string) (ctxt:ctxt) (t:Ll.terminator) : ins list =
  match t with
  | Ret (Void, None) ->  [
      (Movq, [Reg Rbp; Reg Rsp]); (* Restore stack pointer, RSP *)
      (Subq, [Imm (Lit (Int64.mul 6L 8L)); Reg Rsp]); (* Move down for 6 regs *)
      (* Pop regs *)
      (Popq, [Reg R15]); (Popq, [Reg R14]); (Popq, [Reg R13]); (Popq, [Reg R12]);
      (Popq, [Reg Rbp]); (Popq, [Reg Rbx]);
    ]
  | _ -> failwith "unimplemented"


(* Helper function for compile_block:
   Compile list of instructions recursively
*)
let rec compile_insn_list (ctxt: ctxt) (insns : (lbl*insn) list) : X86.ins list =
  match insns with
  | head :: tl -> (compile_insn ctxt head) @ (compile_insn_list ctxt tl)
  | _ -> []
...
let compile_block (fn:string) (ctxt:ctxt) (blk:Ll.block) : ins list =
  if blk.insns = [] then (* Empty block, compile terminator *)
    match blk.term with _, t ->
      compile_terminator fn ctxt t
  else (* Otherwise, compile block instructions *)
    match blk.term with _, t ->
      (compile_insn_list ctxt blk.insns) @ (compile_terminator fn ctxt t)
